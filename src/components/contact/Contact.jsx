...
const Contact = () => {
  ...
  return (
    <section
      id="contact"
      ref={ref}
      className="w-full min-h-screen dark:text-slate-500 relative"
    >
      <div
        style={{ display: resp ? "flex" : "none" }}
      ...
      >
        <p className="text-center text-lg sm:text-2xl md:text-3xl lg:text-4xl">
          LET'S MAKE SOMETHING TOGETHER
        </p>
        <p className="text-md px-1 sm:text-lg text-center">
          Give Me a heads up if you're interested for an interview
        </p>
      </div>

      <Header textArr={contactArr} size="2xl" align="center" />
      <Header
        textArr={interviewArr}
        size="md"
        align="center"
        marginY3X={true}
      />
      ...
      <Animation>
        <form action="" className="p-4" onSubmit={onhandleSubmit}>
          ...
          <motion.div className="my-4" variants={variants}>
            <textarea
              name="message"
              id="message"
              rows="5"
        ...
        <div className="flex justify-start items-center py-2 gap-2">
          <a
            ...
            href="www.google.com"
            className="w-8 h-8 hover:bg-blue-500 rounded-full text-2xl flex justify-center items-center"
          >
            <BsTelegram />
          </a>
